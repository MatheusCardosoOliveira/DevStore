...
app.post('/produto', async (req, resp) => {
    try {    
        ...
        if(nome == '' || categoria == '' || precode == '' || precopor == '' || avaliacao == '' || descricao == '' || estoque == '' || linkimagem == '') {
            return resp.send({ erro: 'Você deve preencher todos os campos' }) 
        }
        if(nome.length <= 4 || categoria.length <= 4 || descricao.length <= 4 || linkimagem.length <= 4) {
            return resp.send({ erro: 'Coloque mais de 4 caracteres nos campos abaixo' })
        }

        if(!isNaN(precopor) == false) {
            return resp.send({ erro: 'Coloque apenas numeros em Preço POR !' })
        }   

        if(!isNaN(avaliacao) == false) {
            return resp.send({ erro: 'Coloque apenas numeros em Avaliação!' })
        }

        if(!isNaN(precode) == false)
            return resp.send({ erro: 'Coloque apenas numeros em Preço DE !' })

        if( !isNaN(estoque) == false)
            return resp.send({ erro: 'Coloque apenas numeros em Estoque!' })
